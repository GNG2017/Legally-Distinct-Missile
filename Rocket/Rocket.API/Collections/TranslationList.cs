...
namespace Rocket.API.Collections
{
    ...
    public class TranslationList : IDefaultable, IEnumerable<TranslationListEntry>
    {
        ...
        public IEnumerator<TranslationListEntry> GetEnumerator() => translations.GetEnumerator();
        IEnumerator IEnumerable.GetEnumerator() => translations.GetEnumerator();

        public void Add(object o) => translations.Add((TranslationListEntry)o);

        public void Add(string key, string value) => translations.Add(new TranslationListEntry(key, value));

        public void AddRange(IEnumerable<TranslationListEntry> collection) => translations.AddRange(collection);

        public void AddRange(TranslationList collection) => translations.AddRange(collection.translations);

        public string this[string key]
        {
            get => translations.Where(k => k.Id == key).Select(k => k.Value).FirstOrDefault();
            set => translations.ForEach(k => { if (k.Id == key) { k.Value = value; } });
        }
